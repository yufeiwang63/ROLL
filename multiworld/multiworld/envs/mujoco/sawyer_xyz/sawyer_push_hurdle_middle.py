...
class SawyerPushAndReachHurdleMiddleXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def model_name(self):
        if(self.puckInvisible == True):
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_hurdle_middle_puck_invisible.xml')
        else:
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_hurdle_middle.xml')
...
class SawyerPushAndReachHurdleMiddleXYEasyEnv_PuckInvisible(SawyerPushAndReachHurdleMiddleXYEnv):
    """
    Always start the block in the same position, and use a 40x20 puck space
    """
