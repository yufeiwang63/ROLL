...
class SawyerPushAndReachXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def __init__(
        ...
        # print("self.reset_free is: ", self.reset_free)

    @property
    def model_name(self):
        if(self.puckInvisible == True):
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_puck_invisible.xml')
        else:
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden.xml')
    ...
    def reset(self):
        if not self.reset_free:
            ...
            # print("initial reset! reset puck!")
            self.reset_mocap_welds()
        else:
            ...
            handreset_pos[:2] = self.get_puck_pos()[:2] - 0.05
            handreset_pos[:2] = np.clip(handreset_pos[:2], self.hand_goal_low, self.hand_goal_high)
            for _ in range(10):
...
class SawyerPushAndReachXYEasyEnv_PuckInvisible(SawyerPushAndReachXYEnv):
    """
    Always start the block in the same position, and use a 40x20 puck space
    """
...
class SawyerPushAndReachXYFullEnv(SawyerPushAndReachXYEnv):
    """
    Fixed initial position, all spaces are 40cm x 20cm
    """
    ...
    def __init__(
        ...
        SawyerPushAndReachXYEnv.__init__(
            self,
            hand_goal_low=(-0.3, 0.5),
            hand_goal_high=(0.3, 0.7),
            puck_goal_low=(-0.3, 0.7),
            puck_goal_high=(0.3, 0.9),
            mocap_low=(-0.25, 0.5, 0.0),
            mocap_high=(0.25, 0.9, 0.5),
            **kwargs
        )

    def sample_puck_xy(self):
        return np.array([0, 0.55])
