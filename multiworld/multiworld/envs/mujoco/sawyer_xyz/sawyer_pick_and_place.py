...
class SawyerPickAndPlaceEnv(MultitaskEnv, SawyerXYZEnv):
    def __init__(
            ...
            p_obj_in_hand=.75,
            ballInvisible = False,
            ballAndRobotInvisible=False,

            **kwargs
    ):
        self.ballInvisible = ballInvisible
        self.ballAndRobotInvisible = ballAndRobotInvisible
    ...
    def model_name(self):
        if self.ballInvisible == True:
            return get_asset_full_path('sawyer_xyz/sawyer_pick_and_place_ball_invisible.xml')
        if self.ballAndRobotInvisible == True:
            return get_asset_full_path('sawyer_xyz/sawyer_pick_and_place_ball_and_robot_invisible.xml')
        else:
