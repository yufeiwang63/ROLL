...
class SawyerPushAndReachTXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def model_name(self):
        if(self.puckInvisible == True):
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_T_puck_invisible.xml')
        else:
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_T.xml')
...
class SawyerPushAndReachTXYEasyEnv_PuckInvisible(SawyerPushAndReachTXYEnv):
    """
    Always start the block in the same position, and use a 40x20 puck space
    """
