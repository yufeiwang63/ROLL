...
class SawyerPushAndReachHurdleResetFreeXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    INIT_HAND_POS = np.array([0.025, 0.55, 0.02])
    ...
    def model_name(self):
        if(self.puckInvisible == True):
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_hurdle_reset_free_puck_invisible.xml')
        else:
            return get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_mocap_goal_hidden_hurdle_reset_free.xml')
    ...
    def reset(self):
        if not self.reset_free:
            ...
            # if reset-free, we do not reset the position of the puck.
            puck_pos = self.get_puck_pos()
            velocities = self.data.qvel.copy()
            angles = np.array(self.init_angles)
            angles[7:10] = puck_pos
            self.set_state(angles.flatten(), velocities.flatten())

            handreset_pos = np.zeros(3)
            handreset_pos[-1] = 0.02
            handreset_pos[:2] = self.get_puck_pos()[:2] - 0.01
            handreset_pos[:2] = np.clip(handreset_pos[:2], self.mocap_low[:2], self.mocap_high[:2])
            
            for _ in range(10):
                self.data.set_mocap_pos('mocap', handreset_pos)
                self.data.set_mocap_quat('mocap', np.array([1, 0, 1, 0]))
    ...
    def init_angles(self):
        ...
        return [1.78026069e+00, - 6.84415781e-01, - 1.54549231e-01,
                2.30672090e+00, 1.93111471e+00, 1.27854012e-01,
                1.49353907e+00, 0, 0.6,
                0.02, 9.99999990e-01, 3.05766105e-05,
                - 3.78462492e-06, 1.38684523e-04, - 3.62518873e-02,
                6.13435141e-01, 2.09686080e-02, 7.07106781e-01,
                1.48979724e-14, 7.07106781e-01, - 1.48999170e-14,
                0.0, 0.61, 0.02,
...
class SawyerPushAndReachHurdleXYResetFreeEasyEnv(SawyerPushAndReachHurdleResetFreeXYEnv):
    """
    ...
    def __init__(
        ...
        SawyerPushAndReachHurdleResetFreeXYEnv.__init__(
            self,
            **actual_kwargs
        )

    def sample_puck_xy(self):
        return np.array([0.025, 0.63])

class SawyerPushAndReachHurdleXYResetFreeEasyEnv_PuckInvisible(SawyerPushAndReachHurdleResetFreeXYEnv):
    """
    Always start the block in the same position, and use a 40x20 puck space
    """
...
class SawyerPushAndReachHurdleXYResetFreeHarderEnv(SawyerPushAndReachHurdleResetFreeXYEnv):
    """
