...
class VAEWrappedEnv(ProxyEnv, MultitaskEnv):
    ...
    def __init__(
        ...
        print("VAE wrapper environment called")
    ...
    def _update_obs(self, obs):
        image = obs[self.vae_input_observation_key]

        # puck_pos = self.wrapped_env.get_puck_pos()[:2]
        # # if puck_pos[0] < 0.1 and puck_pos[0] > 0.0 and puck_pos[1] >= 0.57 and puck_pos[1] <= 0.67:
        # if puck_pos[0] < 0.06 and puck_pos[0] > 0.02 and puck_pos[1] >= 0.645:
        # # if 10 < self.t < 20:
        #     image = drop_puck(image)
        #     # print("dropping puck in image")
        # # print("not dropping image!")
        # obs['image_observation'] = image

        latent_obs = self._encode_one(image)
