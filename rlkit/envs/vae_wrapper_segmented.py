...
class VAEWrappedEnvSegmented(ProxyEnv, MultitaskEnv):
    ...
    def __init__(
        ...
        vae_segmented, # segmeneted images
        segmentation=True,
        ...
        self.input_channels = self.vae_original.input_channels
        ...
        latent_space_goal = Box(
            -10 * np.ones(obs_size or self.vae_segmented.representation_size),
            10 * np.ones(obs_size or self.vae_segmented.representation_size),
            dtype=np.float32,
        )
        ...
        # NOTE yufei: we only differentiate between the segmented and non-segmented observations;
        ...
        self.vae_input_observation_key_segmented = vae_input_key_prefix + '_observation' + '_segmented'
    ...
    def step(self, action):
        ...
        # print("new obs.keys(): ", new_obs.keys())
        self._update_info(info, new_obs) # TODO: update this later
    ...
    def _update_obs(self, obs):
        if self.segmentation:
            ...
            segmented_obs = (obs['image_observation'])
        ...
        # obs['image_observation'] = copy.deepcopy(segmented_obs)

        # self.show_obs(obs['image_observation'], 'original')
        # self.show_obs(obs['image_observation_segmented'], 'segmented')

        assert self.vae_input_observation_key == 'image_observation'
        assert self.vae_input_observation_key_segmented == 'image_observation_segmented'
        latent_obs_ori = self._encode_one(obs[self.vae_input_observation_key], self.vae_original) 
        latent_obs_segmented = self._encode_one(obs[self.vae_input_observation_key_segmented], self.vae_segmented) 
    ...
    def _update_info(self, info, obs):
        # TODO: this info assmues using the original vae for now.
        latent_distribution_params = self.vae_original.encode(
    ...
    def sample_goals(self, batch_size):
        ...
        # print("self.goal sampling mode: ", self._goal_sampling_mode)
        # assert self._goal_sampling_mode == 'vae_prior'
        # print("env.sample_goals called. self._goal_sampling_mode is: ", self._goal_sampling_mode)
        ...
        if self._goal_sampling_mode == 'custom_goal_sampler':
            ...
            if 'image_desired_goal' in sampled_goals:
                ...
                # TODO : needs to segment the image
                sampled_goals['latent_desired_goal'] = self._encode(sampled_goals['image_desired_goals_segmented'], self.vae_segmented)
            return sampled_goals
        elif self._goal_sampling_mode == 'env':
            goals = self.wrapped_env.sample_goals(batch_size)
            # TODO : needs to segment the image
            latent_goals = self._encode(goals[self.vae_input_desired_goal_key], self.vae_segmented)
            ...
            # self.show_obs(goal[self.vae_input_desired_goal_key])
            latent_goals = self._encode(
                goals[self.vae_input_desired_goal_key], self.vae_segmented
            )
        elif self._goal_sampling_mode == 'vae_prior':
            # NOTE yufei: for RIG we actually use this. And this is made correct here.
            # print("online vae segmented using vae_segmented to sample goals")
            # print("using vae_prior to sample goals!!!!!")
            goals = {}
            latent_goals = self._sample_vae_prior(batch_size, self.vae_segmented)
        else:
            raise RuntimeError("Invalid: {}".format(self._goal_sampling_mode))

        if self._decode_goals:
            decoded_goals = self._decode(latent_goals, self.vae_segmented)
        else:
            decoded_goals = None
        ...
        # assert image_goals is not None
        ...
        if image_goals is not None:
            # print("image goals is not None", image_goals)
            goals['image_desired_goal'] = image_goals
        if decoded_goals is not None:
            # print("decoded goals is not None")
            # print("self.vae_input_desired_goal_key is: ", self.vae_input_desired_goal_key)
    ...
    def compute_reward(self, action, obs):
        # achieved_goal = obs['latent_achieved_goal']
        # desired_goal = obs['latent_desired_goal']
        # decoded_achieved_goal = self._decode(achieved_goal, self.vae_segmented)
        # decoded_desired_goal = self._decode(desired_goal, self.vae_segmented)

        # import cv2
        # self.show_obs(decoded_achieved_goal, 'decoded_achieved_goal')
        # self.show_obs(decoded_desired_goal, 'decoded_desired_goal')

        # decoded_achieved_goal = (decoded_achieved_goal * 255).astype(np.uint8)
        # decoded_achieved_goal = decoded_achieved_goal.reshape(3, 48, 48).transpose()
        # decoded_achieved_goal = decoded_achieved_goal[::-1, :, ::-1]
        # cv2.imshow("decoded_achieved_goal", decoded_achieved_goal)
        # cv2.waitKey()


        # decoded_desired_goal = (decoded_desired_goal * 255).astype(np.uint8)
        # decoded_desired_goal = decoded_desired_goal.reshape(3, 48, 48).transpose()
        # decoded_desired_goal = decoded_desired_goal[::-1, :, ::-1]
        # cv2.imshow("decoded_desired_goal", decoded_desired_goal)
        # cv2.waitKey()
    ...
    def compute_rewards(self, actions, obs, show=False):
        ...
        if self.reward_type == 'latent_distance':
            # print("using latent distance to compute rewards")
            achieved_goals = obs['latent_achieved_goal']
            desired_goals = obs['latent_desired_goal']

            # print("in compute rewards, achieved_goals.shape: ", achieved_goals.shape)
            # print("in compute rewards, desired_goals.shape: ", desired_goals.shape)
            assert achieved_goals.shape[1] == self.vae_segmented.representation_size
            assert desired_goals.shape[1] == self.vae_segmented.representation_size

            if show is True:
                # achieved_goals_decoded = self._decode(achieved_goals, self.vae_segmented)
                # desired_goals_decoded = self._decode(desired_goals, self.vae_segmented)
    ...
    def _sample_vae_prior(self, batch_size, vae=None):
        # print("in sample vae prior, vae is: ", vae)
        if vae is None:
            vae = self.vae_original
        if self.sample_from_true_prior:
            # print("sample from true prior")
            mu, sigma = 0, 1  # sample from prior
        else:
            mu, sigma = vae.dist_mu, vae.dist_std
        n = np.random.randn(batch_size, vae.representation_size)
        return sigma * n + mu
    ...
    def _reconstruct_img(self, flat_img, vae=None):
        if vae is None:
            vae = self.vae_segmented
        if vae == self.vae_segmented:
            flat_img = self.segment_obs(flat_img)

        latent_distribution_params = vae.encode(ptu.from_numpy(flat_img.reshape(1,-1)))
